...
// pull in Mongoose model for listItems
...
router.get('/list-items', requireToken, (req, res, next) => {
  ListItem.find()
    .then(listItems => {
      // `listItems` will be an array of Mongoose documents
      // we want to convert each one to a POJO, so we use `.map` to
      // apply `.toObject` to each one
      return listItems.map(listItem => listItem.toObject())
    })
    // respond with status 200 and JSON of the listItems
    .then(listItems => res.status(200).json({ listItems: listItems }))
    // if an error occurs, pass it to the handler
    .catch(next)
})

// SHOW
// GET /listItems/5a7db6c74d55bc51bdf39793
router.get('/list-items/:id', requireToken, (req, res, next) => {
  ...
  ListItem.findById(req.params.id)
    ...
    .then(listItem => res.status(200).json({ listItem: listItem.toObject() }))
    // if an error occurs, pass it to the handler
    .catch(next)
})
...
// UPDATE
// PATCH /listItems/5a7db6c74d55bc51bdf39793
...
// DESTROY
// DELETE /listItems/5a7db6c74d55bc51bdf39793
